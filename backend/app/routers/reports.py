...
async def _validate_export_scope(
    ...
    if normalized_department.lower() == "all" and final_limit >= MAX_REPORT_EXPORT_ROWS:
        raise HTTPException(
            ...
            detail="All-department report exports are limited to smaller bounded extracts. Please narrow the scope or reduce the limit.",
        )
...
async def generate_report(request: Request, payload: ReportFilter):
    ...
    download_url = f"/reports/download?{query_string}"
